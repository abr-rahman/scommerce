        ...
        return datatables()
            ...
            ->editColumn('status', function ($row) {
                if ($row->status == StatusEnum::Active->value) {
                    $html = '<div class="col-sm-5"><a href="' . route('shippings.active', $row->id) . '" class="btn btn-success" id="check_status"></a> </div>';
                    return $html;
                } 
                if ($row->status == StatusEnum::Inactive->value) {
                    $html = '<div class="col-sm-5"><a href="' . route('shippings.inactive', $row->id) . '" class="btn btn-danger" id="check_status"></a> </div>';
                    return $html;
                }
            })
            ...
            // ->editColumn('upazila', function ($row) {
            //     return $row->upazila->name ?? null;
            // })
            ->addColumn('action', function ($row) {
                ...
                $html .= '<a class="dropdown-item edit-btn"  href="' . route('shippings.edit', $row->id) . '">Edit</a>';
                $html .= '<a class="dropdown-item delete-btn" href="' . route('shippings.destroy', $row->id) . '">Delete</a>';
                $html .= '</div>';
                $html .= '</div>';
