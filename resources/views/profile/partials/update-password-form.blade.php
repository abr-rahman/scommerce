<section>
    ...
    <form method="post" action="{{ route('password.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <label class="" for="current_password"><small>{{ __('Current Password') }}</small></label>
        ...
        <div>
            <label class="" for="password" ><small>{{  __('New Password')  }}</small></label>
            <x-text-input id="password" name="password" type="password" class="form-control" autocomplete="new-password" />
            <x-input-error :messages="$errors->updatePassword->get('password')" class="mt-2" />
        </div>

        <div>
            <label class="" for="password_confirmation"><small>{{ __('Confirm Password') }}</small></label>
